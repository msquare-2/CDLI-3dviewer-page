...
var helpON = 0;		// by default help overlay is off
var bgcolor = 'black';	// change background color here

/* 
*function to set up iconbar, scene, loading models call and listen for mose events. 
*/
...
$(function () {
	...
	var objectID = document.getElementById("cdlilink").innerText;
	var importModels = [{name:objectID, xoffset:0}];


	//resize function call and camera setup.
	resize();
	...
	camera.add(directionalLight);// adding directional light to camera
    directionalLight.position.set(150, 150, -40);
	scene.add(camera);// adding camera to scene
	...
	// Mouse handlers
	$('#canvas').on('mousedown', function (mouseData) {
		mouseData.preventDefault();
		onMouseDown(mouseData);
	});
	$('#canvas').on('touchstart', function (mouseData) {
		mouseData.preventDefault();
	...
	$('#canvas').mouseup(function () { isMouseDown = false; isPinching = false; });
	$('#canvas').mouseleave(function () { isMouseDown = false; isPinching = false; });
	$('#canvas').on('touchmove mousemove', function (mouseData) {
		mouseData.preventDefault();
		if (isPinching) {
	...
	$('#canvas').on('mousewheel', function (mouseData) {
		mouseData.preventDefault();
...
function onFullScreenChange() {
	if (!document.fullscreenElement && !document.mozFullScreenElement &&
		...
		// Set to normal according to view port
		resize();
...
/**
 * function to open and close help section
 */
...
function animate() {
   ...
   controls[0].update();    // INERTIA FUNCTION CALL	
...
// Function to resize viewer according to view port 
function resize() {
